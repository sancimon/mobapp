...
private const val TAG = "EditExpenseCategoryDialog"

@Composable
public fun EditExpenseCategoryDialog(
    dialogOpen: MutableState<Boolean>,
    category: ExpenseCategory,
    editCategory: (category: ExpenseCategory) -> Unit,
) {
    val name = remember {
    ...
    val initials = remember {
        mutableStateOf(Util.extractInitials(category.name))
    }

    val hue = remember {
        mutableFloatStateOf(category.tint ?: 0f)
    }
    ...
    Box(modifier = Modifier.fillMaxSize()) {
        AlertDialog(
            ...
            text = {
                val textColor = Util.getExpenseCategoryIconTextColor(hue.value)
                ...
                Column {
                    ...
                    HueBar(setColor = {
                            selectedHue -> hue.value = selectedHue
                    })
                    Text(text = "Appearance:", modifier = Modifier.padding(top = 16.dp, bottom = 8.dp ))
                    CircleIcon(
                        color = Color.hsv(hue.value, 1f, 1f),
                        text = initials.value,
                        textSize = 12.sp,
                        textColor = textColor,
